...
- **Segunda opinião independente** (spec §10b): um modelo não-autor (Groq `openai/gpt-oss-120b` → Gemini `gemini-2.0-flash` fallback, via `llm_client.py`; chaves `api_key_groq`/`api_key_gemini` de `api_keys.txt` ou env `GROQ_API_KEY`/`GEMINI_API_KEY`), **3 personas prompted**: **value** (Graham/Klarman), **growth** (Fisher/Lynch), **contrarian** (bear-first). Cada uma devolve JSON estrito `{verdict, conviction_0_100, one_liner}` na escala **0–100, 50=neutral, 100=buy now**.
...
- `stale: true` → duas metades:
  1. **Python**: `python "%SCRIPTS%\macro_snapshot.py" --fetch` → índices/VIX/yields/FX/commodities/BTC com Δ1d/Δ1w → `_macro/{today}.json`. **Depois** (v4 Phase D) `python "%SCRIPTS%\macro_breadth.py" --update` → funde as keys aditivas `breadth` (RSP/SPY) + `sectors` (11 SPDR ETFs + linha SPY) no mesmo `_macro/{today}.json` (**nunca toca em `metrics`** — overlay-only; cada símbolo mau vira `{"error": ...}`, uma falha nunca aborta a tabela). **Depois ainda** (v4.3.1, roadmap R2) `python "%SCRIPTS%\macro_fred.py" --update` → funde a key aditiva `regime` (M2 via FRED `M2SL` + Buffett Indicator via `NCBEILQ027S`÷`GDP`) no mesmo ficheiro. As unidades das séries são **lidas e convertidas explicitamente** — `NCBEILQ027S` vem em milhões e `GDP` em milhares de milhões, e multiplicar directo dá um Buffett Indicator errado por 1000×; unidade desconhecida ⇒ erro, nunca um número. O rácio usa o **trimestre que ambas as séries cobrem** (a perna das acções atrasa-se um trimestre face ao GDP).
  2. **LLM**: correr `prompts\macro_daily.md` com `{PYTHON_METRICS_JSON}` = o bloco `metrics`, `{BREADTH_JSON}` = o bloco `breadth`, `{SECTORS_JSON}` = o bloco `sectors`, `{REGIME_JSON}` = o bloco `regime` (todos ground-truth — formatar, nunca alterar), `{COUNTRY_TABLE_FRESH}` da directive, `{PREVIOUS_MACRO_MD}` = conteúdo do `fallback_md` (ou "none"). WebFetch **apenas** para §2 e §5, que são as únicas secções ainda sourced: §2 valuation vs história (P/E / forward P/E / P/S / P/B / **EV/EBITDA** / Shiller CAPE com mediana — **uma linha por gauge SEMPRE, mesmo que "not available"**; nunca omitir a linha, senão não se distingue de um gauge que ninguém procurou) e a tabela country. **§6 deixou de ser WebFetch** — vem do `{REGIME_JSON}`, e é por isso que passou a ter números: pedia-se a um LLM que fosse buscar o que uma API fixa já serve. **§7 continua "not available" e isso é um facto registado, não uma falha**: estimativas de lucros futuros ao nível do índice são produto licenciado (FactSet, LSEG, S&P) e nenhuma API gratuita as publica — roadmap **N6**; imprimir o `forward_profit_note` para a razão viajar com o buraco. Tabela country (US/EU/China/Japão) só re-fetch quando `country_table_fresh: no` (TTL 7 dias); caso contrário copiar do anterior. Escrever `_macro/{today}.md` (8 secções) com frontmatter (`date`, `country_table_date`, `sources`, `schema_version`).
